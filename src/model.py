...
class RWKV(pl.LightningModule):
        ...
        for step in range(n_steps):
            ...
            for block_idx, block in enumerate(self.blocks):
                ...
                if args.grad_cp == 1:
                    ...
                    #current_input, current_v_first = deepspeed.checkpointing.non_reentrant_checkpoint(block, current_input, current_v_first)
                    current_input, current_v_first = torch.utils.checkpoint.checkpoint(
                        block, current_input, current_v_first, use_reentrant=False,
                        # 可能需要添加 preserve_rng_state=False，如果遇到 RNG 状态问题
                    )
                else:
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Define depths to evaluate, similar to recurrent-pretraining.py
        # These should ideally come from args, but we'll hardcode for now
        # Example: evaluate at depth 1 and a default/mean depth (e.g., 1)
        # We need a way to access model config's mean recurrence or define it in args.
        # For now, let's assume a fixed list or a single default depth.
        eval_depths = getattr(args, 'eval_depths', [1]) # Default to depth 1 if not specified
        
        losses = {}        
        for depth in eval_depths:
            # Call forward with specified recursive_depth
            logits = self(idx, recursive_depth=depth)
            loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1))
            
            # Log loss for this depth
            # Pytorch Lightning automatically aggregates and logs values returned from validation_step
            # We might need a custom aggregation if we want mean loss across depths later.
            loss_key = f'val_loss_depth_{depth}'
            ppl_key = f'val_ppl_depth_{depth}'
            losses[loss_key] = loss
            losses[ppl_key] = torch.exp(loss)

        # Log the collected losses for different depths
        self.log_dict(losses, on_step=False, on_epoch=True, prog_bar=True)
        
        # Return a representative loss, e.g., loss at the default/first depth
        # Or potentially an average, but PL handles logging dicts well.
        return losses[f'val_loss_depth_{eval_depths[0]}'] # Return loss for the first depth for PL's internal use

    def validation_epoch_end(self, outputs):
        # Optional: Calculate and log average metrics across all validation steps/batches if needed
        # Example: avg_loss = torch.stack([x['val_loss_depth_1'] for x in outputs]).mean()
        # self.log('avg_val_loss_depth_1', avg_loss)
        pass # Basic logging is handled by log_dict in validation_step
