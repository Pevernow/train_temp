...
def MyFunction(fn): return fn # Disable JIT
def MyStatic(fn): return fn # Disable JIT
...
if USE_CUDA_KERNEL:
    ...
    load(name="wkv7", sources=["cuda/wkv7_op.cpp", f"cuda/wkv7.cu"], is_python_module=False,
                        verbose=True, extra_cuda_cflags=["-res-usage", "--use_fast_math", "-O3", "--extra-device-vectorization", f"-D_N_={HEAD_SIZE}"])
...
class RWKV(nn.Module):
    ...
    def forward(self, idx, recursive_depth=1):
        B, T = idx.size()
        # assert T <= args.ctx_len, "Cannot forward, model ctx_len is exhausted." # ctx_len not available here

        x = self.emb(idx)

        # Initialize v_first state
        ...
        for step in range(n_steps):
            ...
            for block_idx, block in enumerate(self.blocks):
                ...
                # Note: grad_cp is not used in demo, so no checkpointing needed
            ...
            # step_v_first is updated implicitly by the loop
